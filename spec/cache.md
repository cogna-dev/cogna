...
- `manifest.json`
- `declarations.ndjson`
- `software-components.ndjson`（可选，仅包含依赖信息的包才存在）
- `metadata.json`
- `checksums.txt`
...
| `name` | ✅ | 组件名 |
| `version` | 可选 | 版本号 |
| `ecosystem` | 可选 | 生态，如 `npm` / `cargo` / `go` |
| `scope` | 可选 | `direct` / `transitive` / `dev` 等 |
| `purl` | 可选 | Package URL，用于生态识别 |

以下字段**不应写入 bundle**：

- `hashes`：bundle 整体已有 `checksums.txt`，组件级 hash 冗余
- `componentType`：可从 `purl` / `ecosystem` 推断
- `location`：软件组件无需 source location
- `signature`：冗余字段
...
- `software-components.ndjson`（可选，字段已按 §4.5 裁剪）
- `metadata.json`
- `checksums.txt`
