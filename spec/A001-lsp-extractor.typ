...
#table(
  ...
  [适用范围], [优先 Go / Rust], [Terraform 仅可作为补充；OpenAPI 仍应以规格解析为主],
)
...
#table(
  ...
  [公开符号大纲], [`textDocument/documentSymbol`], [`symbols.ndjson.zst`、`declarations.path`、`canonical_name`], [为 `query.outlines` / MCP 大纲浏览提供候选列表；为 `build` 生成符号树和作用域路径],
...
#table(
  ...
  [`build`], [直接使用], [启动 LSP、读取符号/定义/hover 等结果，并把它们归一化为 `declarations.ndjson.zst`、`symbols.ndjson.zst`、`metadata.json`],
...
LSP 提取器不是对所有 profile 一视同仁的强依赖。不同生态的收益与必要性不同。

#table(
  columns: (18%, 18%, 28%, 36%),
  inset: 6pt,
  stroke: 0.4pt,
  [*Profile*], [*是否建议启用 LSP*], [*主要收益*], [*权威数据来源*],
  [`go-module`], [是], [公开符号、方法集、定义位置、hover 文档、类型关系], [Go 源码 + `gopls` 补充语义信息],
  [`rust-crate`], [是], [public item、trait item、impl 关系、类型定义、签名与文档], [Rust 源码 + `rust-analyzer` 补充语义信息],
  [`terraform-module`], [可选], [可补充符号与位置，但对变量/输出/provider 约束的结构化抽取帮助有限], [HCL/模块原始文件应保持权威，LSP 仅作辅助],
  [`openapi-spec`], [否], [OpenAPI 已是结构化规格文本，LSP 的附加价值很低], [OpenAPI 文档本体是唯一权威来源],
)

这意味着首期实现可以采取分层策略：

- #strong[Go / Rust]：把 LSP 作为正式增强抽取后端；
- #strong[Terraform]：保留“解析器优先、LSP 可选补充”的策略；
- #strong[OpenAPI]：完全不依赖 LSP，直接解析规格文件。
...
#table(
  ...
  stroke: 0.4pt,
  [*输出项*], [*文件/载体*], [*说明*],
  [声明记录], [`declarations.ndjson.zst`], [逐行写出结构化声明，包含 `id`、`path`、`signature`、`shape`、`docs`、`source_refs`、`language_specific`],
  [符号大纲], [`symbols.ndjson.zst`], [写出公开接口大纲与作用域关系，供 `query.outlines` / MCP 浏览],
...
#table(
  ...
  [`declarations.ndjson.zst`], [主要受益], [更完整的 `signature`、`docs`、`source_refs`、`canonical_name`、`relations`、`shape` 候选信息],
  [`symbols.ndjson.zst`], [主要受益], [基于 document symbols 生成的公开接口大纲、path、kind、summary、owner scope],
...
6. 写出 `declarations.ndjson.zst`、`symbols.ndjson.zst`、`metadata.json` 与 `manifest.json`。
...
- 若语言服务器不可用，则根据 profile 决定：
  - Go / Rust：可默认降级到 parser-only，但需在 `metadata.json` 中明确记录 fallback；
  - Terraform：本就应以 parser-first 为主，降级影响较小；
  - OpenAPI：无影响。
...
#table(
  ...
  [`declarations.ndjson`], [解压并排序后的 `declarations.ndjson.zst`], [作为声明级黄金快照，供 `CodeIQ Extractor` 做字段级对比],
  [`symbols.ndjson`], [解压并排序后的 `symbols.ndjson.zst`], [作为大纲级黄金快照，验证 path / kind / summary / owner scope],
...
3. 读取临时目录下 `dist/manifest.json`、`dist/metadata.json`、`dist/declarations.ndjson.zst`、`dist/symbols.ndjson.zst`；
...
2. #strong[LSP 的价值必须体现在 Bundle 中]：它能提供符号、定义、类型、签名、文档与位置等信息，这些信息应分别进入 `declarations.ndjson.zst`、`symbols.ndjson.zst` 与 `metadata.json`，再被 `diff`、`query`、`check` 与 MCP 间接复用。
