...
#show: arkheion.with(
  ...
  abstract: [
    CodeIQ 是一个面向第三方库与 API 的声明级代码索引和变更治理工具。它从 Go、Rust、Terraform Module 和 OpenAPI 规格中提取公开接口与契约，构建可发布的 `CIQ Bundle`，并通过 diff + check 识别破坏性变更，输出 SARIF 格式的结果，接入现有 CI 与代码审查流程。相较于依赖文档检索或通用代码导航的方案，CodeIQ 聚焦“公开声明”这一更稳定的事实层，并结合单机 Local Registry、CIQ OPA Bundle、本地 SBOM 产物和本地 MCP Agent，为开发者和 AI 提供可追溯、版本绑定、可复用的上下文基础设施。
  ],
...
`Local Registry` 是 CodeIQ 的单机本地实现，也是 `codeiq publish` 的目标。它的职责被有意收敛为两类：#strong[接收并缓存整个 `CIQ Bundle` / `CIQ OPA Bundle`]，以及 #strong[按版本提供本地整包下载 / 物化]。Local Registry 不负责远程执行查询；CLI 与本地 MCP Agent 直接复用同一套本地 bundle store。
...
#table(
  ...
  [Bundle Store], [持久化整包文件], [把 `.ciq.tgz` 作为整体对象管理，默认落在 `.codeiq/cache/registry/bundles`],
...
`codeiq publish` 的推荐行为如下：先本地构建 `CIQ Bundle` 或 `CIQ OPA Bundle`，再将整包写入 Local Registry，由 Local Registry 验证、缓存并生成下载地址。典型返回值为：
...
本项目采用用户故事方式来描述交互流程，并把 `Local Registry` 与 `CodeIQ MCP Agent` 纳入完整使用路径中。
...
该架构的关键原则是：#strong[CodeIQ 只负责抽取和组织声明事实，不负责解释实现逻辑]。Local Registry 负责分发整包与缓存，CLI 负责构建、差异和门禁，MCP Agent 只负责把本地已装载的事实以查询工具形式暴露给模型。这种分层使系统更轻量、更稳定，也更容易和现有治理工具集成。
...
- #strong[针对共享本地 store]：优先把 authoritative storage 固定到 `.codeiq/cache/*`，并通过 focused tests 验证 publish / download / MCP materialization 共享同一套本地事实。
