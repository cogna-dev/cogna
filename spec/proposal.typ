...
本项目围绕五个直接产生或消费标准化产物的核心命令组织；此外，还提供 `publish` 用于把 bundle 上传到 Registry，提供 `mcp start` 用于启动本地查询代理。五个核心命令的输入输出如下：
...
一个典型的使用方式是：SDK 团队先构建并发布 `CIQ OPA Bundle`，随后在业务代码仓库中执行 `codeiq check ./dist/sdk.diff.json --policy ./compat-policy@2026.03.01.ciq.tgz`，把差异结果转成 SARIF。这样，策略本身也和代码一样具备清晰版本、可追溯来源和统一分发路径。
...
在实现上，CodeIQ 可以先做内部规则判定或策略映射，但对外产物统一输出为 `sarifLog`。其中，`changes[].kind` 用于描述变化类型（如 `added`、`removed`、`changed`、`deprecated`），`changes[].level` 用于描述兼容性影响（如 `breaking`、`warning`、`note`）。典型结果映射为：
...
#table(
  ...
  stroke: 0.4pt,
  [*工具名*], [*输入参数*], [*输出*],
  [`codeiq.query.outlines`], [`purl`, `limit?`, `cursor?`], [返回指定包的公开接口大纲索引，包括 path、kind、summary 等信息，供 AI 先做浏览和筛选],
  [`codeiq.query.symbol`], [`purl`, `selector`, `kind?`, `intent?`], [统一返回精确声明、签名、注释、位置，以及 Terraform/OpenAPI 契约摘要],
)

这里将原先的 `codeiq.query.declaration` 与 `codeiq.query.contract` 合并为一个统一工具 `codeiq.query.symbol`。原因是二者本质上都在做“按 selector 查询 bundle 中的结构化声明信息”，差别只在于返回内容更偏函数签名还是更偏配置/API 契约；合并之后，AI 只需要学会一套精确查询接口，既能查询代码声明，也能查询 Terraform/OpenAPI 的契约结构。

同时，不再提供“列出本地 bundle”这类工具，因为 AI 不需要先知道本地缓存里有哪些 bundle；`mcp start` 会根据当前工作目录中的依赖自动发现并装载相关 bundle。新的推荐流程是：#strong[先调用 `codeiq.query.outlines` 根据 PURL 获取公开接口大纲，再调用 `codeiq.query.symbol` 对感兴趣的 path 做精确查询]。其中也不再提供 `resolve` 工具：因为 bundle 的解析与下载属于 CLI/Registry 流程，而不是 MCP 的职责；`include` 这类细粒度远程对象选择参数也随之删除。每个 MCP 工具都应在 `tools/list` 中声明 `inputSchema` 与 `outputSchema`，并在 `tools/call` 结果中返回 `structuredContent`。例如，AI 可以先这样调用 `codeiq.query.outlines`：
...
拿到大纲中的候选 path 之后，再调用 `codeiq.query.symbol` 获取精确声明：
...
- #strong[MCP 合约测试]：只验证 `codeiq.query.outlines` 与 `codeiq.query.symbol` 这类常驻内存查询工具的 schema 与结果结构。
...
我们没有直接采用 LSIF 或 SCIP，是因为这两类格式更偏向代码导航场景，例如 Hover、Definition、References 等 IDE 能力。CodeIQ 的重点则是第三方库的公开接口治理、变更检查、策略门禁和整包分发。为此，CIQ 从一开始更注重声明清晰可读、版本之间便于比较，以及分发和缓存效率，而不是追求完整的代码导航图谱。
