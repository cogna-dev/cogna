...
1. 用户侧 API 收口
2. 当前项目上下文与 `.cogna/sbom.spdx.json` 缓存机制
...
```mermaid
flowchart LR
    A[当前项目路径] --> B[读取 cogna.yml]
    B --> C{.cogna/sbom.spdx.json 是否可用}
    C -- 是 --> D[加载当前项目 SBOM]
    C -- 否 --> E[重新扫描依赖并写入 .cogna/sbom.spdx.json]
    E --> D
    D --> F[构建当前项目相关包树]
    F --> G[Build / Diff]
    F --> H[FetchPackages]
...
- **优先读取** `.cogna/sbom.spdx.json`
- **必要时刷新**：例如 `cogna.yml`、lockfile、manifest、workspace 配置、依赖清单变化后再重算
...
这个接口没有业务参数，因为它完全依赖当前项目上下文和 `.cogna/sbom.spdx.json`。
...
- `buf.gen.yaml` 现已直接调用本地 Go plugin `tools/protoc-gen-cogna/protoc-gen-cogna`
- 同一 generator core 已通过两次 local plugin 调用分别生成 MoonBit 与 Node TS 产物
- `src/sdk/proto/xaclabs/cogna/v1/*.mbt` 已能通过 `moon check`
- Node SDK build 现已把 `integrations/sdk/node/src/generated/*.ts` 编译并发布到 `dist/generated/*`
- `@cogna/sdk` 现已导出 `@cogna/sdk/generated` 子路径供 TS consumer 直接消费 proto-first DTO
...
inputs:
  - directory: proto
plugins:
  - local: tools/protoc-gen-cogna/protoc-gen-cogna
    out: .
    opt:
      - target=moonbit
...
```text
src/sdk/proto/xaclabs/cogna/v1/*.mbt
```
...
2. `integrations/sdk/node/scripts/prepare-runtime.mjs` + package build 流程再通过 `tsc` / `vite` 编译并发布到 `dist/generated/*.js` / `*.d.ts`

这样既保留 repo 内可读的生成源码，也让 npm 包对外暴露稳定的 `@cogna/sdk/generated` 子路径。
...
1. **proto contract version**：例如 `xaclabs.cogna.v1`
2. **generator version**：例如 `cogna-proto-gen v0.1.0`
...
### 12.1 当前 buf 生成链路已切到本地 cogna plugin

当前 `buf.gen.yaml` 已改为两次调用本地 `protoc-gen-cogna`，分别生成 MoonBit 与 Node TS。说明：

- repo 已经接受“buf 负责代码生成”的方向
- 且生成链路已经进入 MoonBit 编译与 Node 发布主链路
- 仍需继续补齐 round-trip/sample consumer 验证与剩余 wrapper surface 收口

### 12.2 当前 Node SDK 已接入生成类型，运行时通过 package-local runtime 装配

当前 `integrations/sdk/node/scripts/prepare-runtime.mjs` 会：

1. 执行 `buf generate --template buf.gen.yaml`
2. 执行 `moon build --target js src/sdk`
3. 把 MoonBit JS runtime 复制到 `integrations/sdk/node/src/runtime/sdk.runtime.js`
4. 由 package build 流程产出 `dist/generated/*.js|.d.ts` 与 wrapper entry
...
MoonBit 侧当前已经有生成链路，且 `src/sdk/proto/xaclabs/cogna/v1` 可通过 `moon check`。因此：
...
- 生成类型如果没有接入 `integrations/sdk/node/scripts/prepare-runtime.mjs` 与 package build 流程，就只是孤立产物

规避策略：

- 在计划里单独列一个“Node packaging integration”阶段
- 验收标准必须包含 `@cogna/sdk` 实际能导出生成类型
...
- `moon check src/sdk/proto/xaclabs/cogna/v1` 通过
...
- `pnpm --filter @cogna/sdk build` 与 `npm run typecheck` 通过
...
- `@cogna/sdk` 的对外导出里包含生成类型（至少 `@cogna/sdk/generated`）
...
- MoonBit / Node SDK e2e：`src/e2e/native/sdk_test.mbt`
  - MoonBit 侧验证 `src/sdk/index`、`src/sdk/query` 与 `src/sdk/proto/xaclabs/cogna/v1`
  - Node 侧验证预构建的 `integrations/sdk/node/dist/*` 与 `@cogna/sdk/generated` DTO surface

## Stage F — SBOM context integration

目标：

- 让用户侧 API 全部建立在“当前项目 + `.cogna/sbom.spdx.json`”之上
...
- `.cogna/sbom.spdx.json` 成为默认上下文入口
...
- 再设计一个 `cogna-proto-gen`，从同一份 proto 生成 MoonBit 与 Node TS 的数据类型
