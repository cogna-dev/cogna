...
#show: arkheion.with(
  title: "CodeIQ: 项目申报书",
  authors: (
    (name: "Yufei Li", email: "yufeiminds@gmail.com", affiliation: "", orcid: ""),
  ),
  abstract: [
    CodeIQ 是一个面向第三方库与 API 的声明级代码索引和变更治理工具。它从 Go、Rust、Terraform Module 和 OpenAPI 规格中提取公开接口与契约，构建可发布的 `CIQ Bundle`，并通过 diff + check 识别破坏性变更，输出 SARIF 格式的结果，接入现有 CI 与代码审查流程。相较于依赖文档检索或通用代码导航的方案， CodeIQ 聚焦“公开声明”这一更稳定的事实层，并结合 CodeIQ Registry、CIQ OPA Bundle 和本地 MCP Agent，为开发者和 AI 提供可追溯、版本绑定、可复用的上下文基础设施。
  ],
  keywords: ("CodeIQ", "CIQ", "SARIF", "OPA", "Registry", "MCP", "Go", "Rust", "Terraform", "OpenAPI"),
  date: "March 19, 2026",
)

= 项目目标与应用场景

CodeIQ 的目标不是做一个新的 linter，也不是替代 LSP/IDE 导航，而是构建一个#strong[面向库公开接口（声明）]的事实提取与变更治理系统。它解决的是这样一类问题：当开发者或 AI 代理面对第三方库升级、SDK 发版、Terraform Module 变更、OpenAPI 规格更新时，现有工具往往只能提供文本检索、通用静态分析或实现级告警，却无法稳定回答“这个产物对外承诺了什么”“两个版本之间哪些公开声明发生了变化”“这些变化是否违反组织的兼容性策略”“结果能否接入现有审查与报告工具链”。
...
本项目不支持 CUE Package，且不把 README、示例项目、外部文档作为输入。#strong[所有事实都仅从代码与规格文件本体中提取]，从而避免“文档过时但工具仍认为正确”的不确定性。

项目定位因此可以概括为：#strong[替代 Context7 的库声明知识底座 + 面向变更治理的结构化事实系统]。它提供的不是泛文档片段，而是带版本、带位置、带唯一标识的声明级证据。

= 交付物说明

== 拟支持的核心功能与功能边界（Scope）

CodeIQ 的功能边界必须和 lint 工具明确区分。lint 面向代码实现逻辑、风格和潜在缺陷；CodeIQ 面向库的#strong[公开接口、契约与差异]。也就是说，CodeIQ 不做控制流、数据流、漏洞利用链与编码风格判断，只回答“这个库对外暴露了什么”以及“它变了什么”。

#table(
  columns: (1.5fr, 2fr, 2.7fr),
  inset: 6pt,
  stroke: 0.4pt,
  [*维度*], [*传统 lint / 静态检查*], [*CodeIQ*],
  [分析对象], [实现逻辑、风格、局部缺陷], [公开接口、声明、版本差异、产物契约],
  [输入], [源码文件、AST、CFG], [代码仓库中的声明与规格文件],
  [输出], [warning / error], [CIQ Bundle / CIQ Diff / SARIF / CIQ Result],
  [典型问题], [是否有未使用变量、空指针风险], [是否删除了公开 API、变更了 schema、破坏了模块输出契约],
  [适用对象], [应用开发与实现质量], [库开发、依赖治理、平台门禁、AI 查询],
)

从用户视角看，CodeIQ 首期提供以下核心能力：
...
- #strong[把热点依赖和规则沉淀到团队基础设施里]：用户可以把 `CIQ Bundle` 与 `CIQ OPA Bundle` 发布到 `CodeIQ Registry`，实现缓存、分发与统一版本治理。
- #strong[在 AI 工具中直接查询依赖和接口]：用户可以通过本地 CLI 或 `CodeIQ MCP Agent` 查询声明、签名、注释与契约信息，减少 AI 对外部文档搜索的依赖。
- #strong[用统一方式治理代码与配置变更]：用户可以对代码语言和配置语言采用一致的工作流，统一管理接口变化、策略门禁和上下文分发。

== 输入输出定义与 CIQ 格式

本项目的软件界面围绕五个命令组织，其输入输出如下：
...
`init` 生成 `codeiq.yaml`，用于定义项目坐标、抽取 profile、入口与策略配置。其目标是让使用者明确告诉系统“这个仓库在什么生态中、哪些文件定义了公开接口、应使用何种检查策略”。
...
`CIQ OPA Bundle` 是一种特殊的 `CIQ Bundle`，专门用于存储和管理与 Open Policy Agent (OPA) 相关的策略和规则。它不是面向代码声明查询的索引，而是面向 `check` 阶段的策略载荷：其中包含 OPA 的策略文件、数据文件以及相关元数据，可以像普通 `CIQ Bundle` 一样被 `CodeIQ Registry` 管理、缓存、下载和版本化。

它的作用是把“检查规则”也纳入 CodeIQ 的版本治理体系：库团队可以把兼容性规则、发布门禁规则、组织约束规则单独构建成策略产物，再由 `codeiq check` 在本地装载执行。这种设计特别适合需要频繁更新和维护策略的项目。
...
一个典型的使用方式是：SDK 团队先构建并发布 `CIQ OPA Bundle`，随后在业务代码仓库中执行 `codeiq check --policy ./compat-policy@2026.03.01.ciq.tgz`，把差异结果转成 SARIF。这样，策略本身也和代码一样具备清晰版本、可追溯来源和统一分发路径。

=== CIQ Diff

`diff` 输入代码仓库（及其基线版本信息），输出 `CIQ Diff`。`CIQ Diff` 记录的不是源文件的逐行变化，而是#strong[声明层]的 added / removed / changed / deprecated / compatibility-risk 事件。
...
`check` 的输入为 `CIQ Bundle` 或 `CIQ Diff`，输出必须是 SARIF，而不是内部 JSON 决策。这样可以直接接入现有代码扫描、代码审查和质量门禁工具。

在实现上，CodeIQ 可以先做内部规则判定或策略映射，但对外产物统一输出为 `sarifLog`。典型结果映射为：
...
`CodeIQ Registry` 是 CodeIQ 的云端实现，也是 `codeiq publish` 的目标。它的职责被有意收敛为两类：#strong[接收并缓存整个 `CIQ Bundle` / `CIQ OPA Bundle`]，以及 #strong[按版本提供整包下载]。Registry 不负责远程执行查询；用户需要先把 bundle 下载到本地，再通过 CLI 或本地 MCP Agent 进行查询。
...
#table(
  ...
  [Object Store], [持久化整包文件], [把 `.ciq.tgz` 作为整体对象管理，不暴露内部实现细节],
  [Bundle Cache], [缓存热点产物], [为频繁下载的 bundle 维持热缓存，降低拉取时间并提升本地查询启动速度],
)
...
这里需要额外说明 #strong[Bundle ID] 的设计原因。Registry 中同一个 bundle 可能同时被多个 tag 或友好版本名引用，例如 `latest`、`v1.43.0` 与某个固定发布时间点标签，都可能指向同一个底层产物；与此同时，用户在可复现的 CI 场景里又希望按不可变内容来引用该产物。因此，Registry 同时保留两套引用方式：
...
换句话说，#strong[多个 tag 可以指向同一个 bundleId，而同一个 bundleId 也可以同时被 tag 和 digest 两种方式引用]。因此 Registry API 采用 `bundleId` 作为内部下载与缓存主键，是为了在“人类友好引用”和“机器稳定引用”之间取得平衡。

`codeiq publish` 的推荐行为如下：先本地构建 `CIQ Bundle` 或 `CIQ OPA Bundle`，再将整包上传到 Registry，由 Registry 验证、缓存并生成下载地址。典型返回值为：
...
```json
{
  ...
  "downloadUrl": "https://registry.codeiq.dev/api/v1/bundles/bundle_sha256_abc123/download"
}
```

对 Registry 而言，#strong[digest 不可变、bundle 整体下载、缓存命中优先] 是三条基本原则。这样既可以支持 SDK 团队将稳定产物发布给他人的 Agent 使用，也能避免 Registry 暴露过多内部细节；查询能力留在本地，从而让云端职责更简单、部署更稳健。

=== CodeIQ MCP Agent

`CodeIQ MCP Agent` 不是 CodeIQ 的万能入口，而是一个#strong[本地常驻内存查询代理]。它的职责非常明确：在本地加载一个或多个 `CIQ Bundle` 进入内存后，为 AI Agent 暴露“查声明 / 查签名 / 查注释 / 查契约”这类高频查询能力。那些不需要常驻内存、且更适合批处理和脚本集成的操作，如 `build`、`diff`、`check`、`publish`，都应通过 CLI 调用，而不是通过 MCP 调用。
...
#table(
  ...
  stroke: 0.4pt,
  [*工具名*], [*输入参数*], [*输出*],
  [`codeiq.bundle.list_loaded`], [无], [当前已装载的本地 bundle 列表、PURL、版本、profile],
  [`codeiq.query.symbol`], [`bundleRef?`, `selector`, `kind?`, `intent?`, `limit?`], [统一返回声明、签名、注释、位置，以及 Terraform/OpenAPI 契约摘要],
)

这里将原先的 `codeiq.query.declaration` 与 `codeiq.query.contract` 合并为一个统一工具 `codeiq.query.symbol`。原因是二者本质上都在做“按 selector 查询 bundle 中的结构化声明信息”，差别只在于返回内容更偏函数签名还是更偏配置/API 契约；合并之后，AI 只需要学会一套查询接口，既能查询代码声明，也能查询 Terraform/OpenAPI 的契约结构。

其中不再提供 `resolve` 工具：因为在新的设计里，bundle 的解析与下载属于 CLI/Registry 流程，而不是 MCP 的职责；`include` 这类细粒度远程对象选择参数也随之删除。每个 MCP 工具都应在 `tools/list` 中声明 `inputSchema` 与 `outputSchema`，并在 `tools/call` 结果中返回 `structuredContent`。例如，`codeiq.query.symbol` 的典型调用可以表示为：
...
```json
{
  ...
  "params": {
    ...
    "arguments": {
      "bundleRef": "pkg:cargo/tokio@1.43.0",
...
正文中不再展开 AI 使用示例，而是建议用户直接通过 `codeiq init --claude-skill` 初始化本地的 Claude Skill。这样，CodeIQ 可以在初始化项目配置时，顺带生成 `.claude/skills/codeiq`，让 Claude Code、OpenCode 等工具后续直接复用这份项目级能力说明。

```bash
codeiq init --claude-skill
```

执行后，用户将同时获得：

- 项目级 `CIQ Config` 初始化结果；
- `.claude/skills/codeiq` 目录或对应 Skill 文件；
- 面向 CLI 与 MCP Tool 的推荐使用约定。

== 预期使用方式或交互流程

本项目采用用户故事方式来描述交互流程，并把 `CodeIQ Registry` 与 `CodeIQ MCP Agent` 纳入完整使用路径中。
...
#figure(
  diagram(
    ...
    node((2, 0), [CodeIQ Registry], corner-radius: 2pt),
...
- #strong[用户故事 2：平台工程师]
  ...
  3. 随后，他执行 `publish` 把这些 bundle 统一发布到 `CodeIQ Registry`，让平台能力以标准化产物的形式沉淀下来。
  4. 下游团队需要使用平台能力时，不再分别翻阅多个仓库，而是先把所需 bundle 下载到本地，再通过 `query` 或 `mcp start` 使用这些产物。
  5. 最终效果是：平台工程师把“代码实现 + 部署运维 + 开放平台契约 + 策略规则”统一交付为可缓存、可下载、可查询的资产，下游团队也能复用同一套上下文来源。
...
#figure(
  diagram(
    ...
    node((1, 1), [CodeIQ MCP Agent], corner-radius: 2pt),
...
- #strong[用户故事 3：后台开发工程师]
  ...
  2. 如果只是一次性查询，他直接执行 `query --bundle <bundle>`，获取指定声明、签名或契约说明；如果是高频交互式查询，他会执行 `mcp start`，让 AI Agent 根据当前包管理器依赖自动发现并装载需要的 bundle，再通过 MCP 反复查询这些本地产物。
...
#figure(
  diagram(
    cell-size: 7mm,
    node-stroke: 0.8pt,
    node((0, 0), [CodeIQ Registry], corner-radius: 2pt),
    node((1, 0), [本地 CIQ Bundle], corner-radius: 2pt),
    node((2, 0), [CodeIQ MCP Agent], corner-radius: 2pt),
...
- #strong[Schema 测试]：验证 `CIQ Config`、`CIQ Bundle`、`CIQ OPA Bundle`、`CIQ Diff`、`CIQ Query`、`CIQ Result` 与 SARIF 输出字段的 schema 合法性。
...
- #strong[策略 Bundle 测试]：验证 `opa-bundle.tar.gz` 的加载、签名校验、版本切换与 `check` 入口点选择是否稳定。
- #strong[Registry 测试]：验证整包上传、版本索引、下载地址、ETag/缓存命中与失效策略。
- #strong[MCP 合约测试]：只验证 `bundle.list_loaded` 与 `query.symbol` 这类常驻内存查询工具的 schema 与结果结构。
...
#table(
  ...
  stroke: 0.4pt,
  [*象限*], [*目标读者*], [*重点内容*], [*典型问题*],
  [Tutorials], [初次接触 CodeIQ 的开发者], [从零完成一次 `init → build → diff → check → publish` 的学习路径], [我第一次使用 CodeIQ，应该按什么顺序走通完整流程？],
...
#table(
  ...
  [云端层], [CodeIQ Registry + Bundle Cache], [提供 publish、版本索引、整包下载与热点缓存能力],
  [Agent 层], [CLI + 本地 MCP Query Agent], [CLI 负责 build/diff/check/publish，MCP 只负责常驻内存查询],
)

该架构的关键原则是：#strong[CodeIQ 只负责抽取和组织声明事实，不负责解释实现逻辑]。Registry 负责分发整包与缓存，CLI 负责构建、差异和门禁，MCP Agent 只负责把本地已装载的事实以查询工具形式暴露给模型。这种分层使系统更轻量、更稳定，也更容易和现有治理工具集成。
...
CodeIQ 之所以重新设计 CIQ，而不是直接复用 LSIF 或 SCIP，是因为目标场景根本不同。LSIF 和 SCIP 的主要目标是服务代码导航、Hover、Definition、References 等 IDE / code intelligence 场景；而 CodeIQ 的核心目标是面向 #strong[第三方库公开接口治理]、#strong[变更检查]、#strong[策略门禁] 与 #strong[整包缓存分发]。这要求格式从一开始就优先考虑“声明可读性、变更可比较性、分发与缓存效率”，而不是优先考虑“图导航完整性”。
...
#table(
  ...
  stroke: 0.4pt,
  [*对比项*], [*LSIF*], [*SCIP*], [*CIQ*],
  [主要目标], [离线承载 LSP 风格代码导航], [更高效地承载精确代码导航索引], [承载公开接口、契约差异、策略门禁与上下文分发],
  [核心数据形态], [图结构与顶点/边], [紧凑二进制索引], [Bundle 化声明记录 + 差异 + 策略 bundle],
  [更适合的查询], [Hover、Definition、References], [精确导航与符号引用], [声明、签名、注释、契约、变更与 policy 结果],
  [对第三方库治理的适配], [间接适配，偏重导航], [间接适配，偏重导航], [直接适配，天然围绕库治理设计],
  [典型体积], [通常是代码体积的 10–30 倍], [通常约为 LSIF 的 1/10], [不包含代码逻辑，通常比代码小一个数量级],
  [性能重点], [导航可用性优先], [索引压缩与传输效率优先], [整包缓存、本地查询启动速度、变更比较效率优先],
  [云端分发方式], [通常作为导航索引资产], [通常作为导航索引资产], [天然支持 Registry 缓存与 Bundle ID 管理],
)

因此，CIQ 的差异化价值不在于“比 LSIF / SCIP 更通用”，而在于 #strong[更适合库接口治理这个明确场景]。它用更小、更稳定的核心字段表达结构化声明，用 `CIQ Diff` 表达接口变化，用 `CIQ OPA Bundle` 表达策略规则，再通过 Registry 与本地 MCP 查询能力组合出一条完整工作流。由于 CIQ 不承载实现逻辑和完整导航图谱，在目标场景中通常可以获得明显更低的存储、缓存与传输成本。
...
对 CodeIQ 而言，选择 SARIF 有两个直接好处：

- #strong[结果可被现有工具链直接消费]：不需要额外设计私有格式的上传接口。
- #strong[规则、结果、位置语义完整]：适合表达“哪条 policy 被违反、发生在什么位置、严重级别如何”。

更重要的是，#strong[GitHub 已明确支持 SARIF 作为代码扫描结果的标准格式]，能够解析第三方工具产出的 SARIF 文件，并将其展示为代码扫描告警。这意味着 CodeIQ 的 `check` 结果可以天然进入 GitHub 与现有代码审查工作流，而不需要额外适配一套自定义展示逻辑。
...
也正因为如此，CodeIQ 不需要重新发明一门策略语言，而是把重点放在“如何把结构化声明与差异结果喂给 OPA”，并通过 `CIQ OPA Bundle` 把策略本身纳入统一版本治理。

=== 为什么要构建 CodeIQ Registry

CodeIQ Registry 的意义，不只是提供一个上传地址，而是为 `CIQ Bundle` 和 `CIQ OPA Bundle` 提供 #strong[性能优化过的缓存与分发层]。如果没有 Registry，每个团队和每个 AI 工具都需要自己重新构建或重复传输同样的 bundle；而有了 Registry，热点产物可以被集中缓存、版本索引并按 Bundle ID 精确分发，从而显著降低下游获取上下文的延迟。

除此之外，Registry 还有两个关键价值：

- #strong[私有化部署]：可以在企业私有环境中部署，避免把内部接口信息和策略产物暴露到公网。
- #strong[缓解软件供应链安全攻击]：通过整包发布、Bundle ID、digest 和缓存控制，用户可以明确知道自己消费的是哪一个不可变产物，降低被篡改、被替换或被错误引用的风险。
...
因此，CodeIQ 支持配置语言并不是“顺手多做一点”，而是因为 #strong[配置语言本身就是软件系统对外承诺的重要组成部分]。如果只分析代码、不分析配置与 API 规格，那么用户仍然无法真正掌握系统的完整变更风险。通过把 Terraform 与 OpenAPI 纳入统一 Bundle、Diff、Check 和 Query 工作流，CodeIQ 才能提供面向真实交付系统的完整上下文能力。
...
- #strong[Registry 的部署与访问压力]：Registry 如果公网部署，可能承接较大的访问压力，并带来额外的性能问题或安全风险。
...
- #strong[针对 Registry 部署]：优先提供私有化部署方案，基于容器化技术帮助用户快速部署到私有环境；同时支持无 Registry 的 fallback 方案，允许用户在本地环境中完成 `build / diff / check / query` 等核心流程。
...
- #strong[MCP]：用于把工具和外部数据源标准化暴露给模型的开放协议，为 CodeIQ MCP Agent 提供工具接口设计基础。
- #strong[Semgrep]：面向规则与模式匹配的轻量静态分析工具，适合作为“CodeIQ 不做实现级分析”的对照参照物。
- #strong[PURL]：跨生态的软件包唯一标识规范，为声明快照、依赖治理与结果交换提供统一身份语义。
- #strong[SARIF]：静态分析结果交换标准，使 CodeIQ 的检查结果可以无缝进入现有工具链。

从技术现状看，SCIP/LSIF 主要解决“导航索引如何表达”的问题，MCP 主要解决“模型如何发现和调用工具”的问题，Semgrep 主要解决“规则与模式如何扫描”的问题，而 CodeIQ 关注的是“库公开接口如何被结构化抽取、比较、发布和查询”。几者并不冲突，但定位不同。CodeIQ 的差异化价值在于：把声明级事实抽取、PURL 标识、版本差异、Registry 缓存、MCP 工具接口和 SARIF 集成，统一到一个轻量、可工程化消费的工作流里。

= 个人背景与项目匹配度

我是李宇飞（yufeiminds），长期从事开发者基础设施、开发者工具与平台工程，持续关注 SDK、CLI、云资源编排与 AI 开发流程中的工程确定性问题。相较于追求“更强大的分析器”，我更关注“如何把关键事实提取出来，并稳定接入开发流程”，这与 CodeIQ 的声明级定位高度一致。
...
这些基础使我能够从工程实现和产品交付两个层面推进 CodeIQ：既理解声明抽取与格式设计，也理解如何把结果真正纳入 CI、质量平台和 AI 工作流。
