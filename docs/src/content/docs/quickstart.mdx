...
> 下面命令与行为基于当前已落地实现（含 OPA CLI 执行、Registry 本地/remote 发布链路、MCP HTTP runtime、diff 可选参数）。
...
如果你希望同一份 bundle 被多人、多个工具或多台机器复用，那么可以把它发布到 Registry。当前默认链路会先写入本地 registry 存储（`.codeiq/registry`）；如果配置 `CODEIQ_REGISTRY_BASE_URL`，则会走 remote / loopback HTTP Registry 路径，并生成不可变 publish receipt。
