---
title: 工作方式
description: 解释 Cogna 现在的本地运行模型：bundle、registry、MCP、SPDX 与 dependency bundles 如何协同。
order: 10
---

# 工作方式

Cogna 当前仍然是**单机闭环**：你在本地仓库里构建 bundle，本地 registry 负责按 PURL 存储和分发，MCP 负责把这些 bundle 暴露给 AI / Agent。
...
flowchart LR
    subgraph BUILD["1. 构建"]
        ...
        B -->|"cogna build"| C2["dist/sbom.spdx.json"]
    ...
    subgraph SHARE["3. 共享"]
        C -->|"cogna publish"| F["Local Registry"]
        C3 -->|"publish.includeDependencies"| F
    end

    subgraph QUERY["4. 查询"]
        F -->|"cogna registry serve"| G["HTTP Registry API"]
        C -->|"cogna query"| H["CLI 单次查询"]
        F -->|"cogna mcp serve"| I["AI / Agent MCP 接口"]
    end
```
