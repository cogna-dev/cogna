---
title: 定制 OPA 自动审查策略
description: 如何把本地 Rego 规则构建成 policy bundle，并用它驱动 Cogna 的 compatibility check，生成 SARIF 报告。
order: 25
---

# 定制 OPA 自动审查策略

Cogna 的兼容性检查是由 OPA（Open Policy Agent）驱动的。你可以用 Rego 语言写自己的策略规则，打包成 policy bundle，然后让 `cogna check` 用它来判断哪些 API 变化是可接受的，哪些需要阻止或警告。
...
flowchart LR
    ...
    C["进入目标仓库\ncogna build"] --> D["准备 dist/base + dist/target\ncogna diff"]
    B --> E["cogna check\n执行 OPA eval"]
    D --> E
...
在生成的 `cogna.yml` 中，把 `profile` 设置为：
...
在 policy 仓库的 `rego/` 目录下创建规则文件。Cogna 的 `check` 命令会执行 `data.cogna.compat.deny`。
...
# 在 cogna.yml 里把 checks.policy 指向 ../policy/dist/bundle.ciq.tgz
cogna check
```

当前 CLI 不暴露 `--policy` 或 `--since` 参数；它们分别由 `cogna.yml -> checks.policy` 和你准备好的 `dist/base` / `dist/target` 决定。
...
Cogna 自带一套 built-in policy，覆盖常见的兼容性场景。你不需要自己维护这些规则，只需要构建 SDK bundle 和执行 `check` 即可。
