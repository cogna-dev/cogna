...
| `tool.driver.rules[].helpUri` | 规则文档链接；built-in rules 会稳定指向 `/docs/policies#rule-<rule-id-with-dashes>` |
...
- 了解 policy 规则编写：[内置策略规则](/docs/policies)
- 在 CI 中运行完整流程：[持续集成](/docs/ci)
- 了解发版检查流程：[分析变更](/docs/analyze-changes)
