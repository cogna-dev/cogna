...
你可能在开发机上构建了一个 SDK 的 bundle，现在想把它分享给其他人——让他们也能用 `codeiq query` 查询，或者让 CI 从一个统一的地方拉取，而不是每次都重新构建。这就是 bundle 共享要解决的问题。

## 核心流程

```mermaid
flowchart LR
    A["codeiq build\n生成 bundle"] --> B["codeiq publish\n发布到本地 registry"]
    B --> C["codeiq registry serve\n启动 HTTP server"]
    C --> D["其他人 / CI\nHTTP 下载 / 按 PURL 解析"]
    B --> E["本机 MCP\ncodeiq mcp serve"]
```
...
PURL 格式是 `pkg:<type>/<namespace>/<name>@<version>`，在 `codeiq.yml` 的 `purl` 字段中配置。
...
| 场景 | 做法 |
|------|------|
| 本机 CLI 和 MCP 复用同一份 bundle | `codeiq publish` + `codeiq mcp serve` |
| 团队内部共享最新 SDK 快照 | `codeiq registry serve` 或 Docker 化 registry + HTTP 下载 |
...
- PURL 格式和 `codeiq.yml` 配置：[配置参考](/docs/config)
