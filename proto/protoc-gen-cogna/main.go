...
import (
	...
	"github.com/cogna-dev/cogna/proto/protoc-gen-cogna/internal/emitter"
	"github.com/cogna-dev/cogna/proto/protoc-gen-cogna/internal/ir"
...
func appendSyntheticMappings(raw string, files []*descriptorpb.FileDescriptorProto) string {
	...
	for _, file := range files {
		pkgPath := path.Join("github.com/cogna-dev/cogna/proto", path.Dir(file.GetName()))
...
func fail(err error) {
	fmt.Fprintf(os.Stderr, "protoc-gen-cogna: %v\n", err)
	os.Exit(1)
}
